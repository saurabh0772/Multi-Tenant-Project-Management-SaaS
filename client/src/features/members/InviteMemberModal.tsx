...
import { X, Loader2, AlertCircle, Check, Copy, Link as LinkIcon } from "lucide-react";
...
export const InviteMemberModal: React.FC<InviteMemberModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await orgApi.inviteMember(orgId, { email, role });
      await queryClient.invalidateQueries({ queryKey: ["invitations"] });
      
      if (res.token) {
        const url = `${window.location.origin}/accept-invitation?token=${res.token}`;
        setInviteUrl(url);
      } else {
        onClose();
      }
        ...
        {inviteUrl ? (
          <div className="space-y-4">
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400 text-xs flex items-center gap-2">
              ...
              <span>Invitation created successfully for <strong>{email}</strong>!</span>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  readOnly
                  value={inviteUrl}
                  className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs font-mono text-slate-300 focus:outline-none"
                />
                <button
                  type="button"
                  onClick={handleCopy}
                  className="px-3.5 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-medium transition-all flex items-center gap-1.5 shrink-0"
                >
                  {copied ? (
                    <>
              ...
              <p className="text-[10px] text-slate-500 mt-1.5">
                Send this secret link to <strong>{email}</strong> to let them join this organization.
              </p>
            </div>
